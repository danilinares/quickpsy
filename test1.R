...
fit_main <- function(d, x, k, n, guess, lapses, fun) {
  x <- deparse(substitute(x))
  k <- deparse(substitute(k))
  n <- deparse(substitute(n))
  psy_fun <- create_psy_fun(guess,lapses,fun)
  nll <- create_nll(d, x, k, n, psy_fun)
  if (lapses) optim(c(.7, .7,.2), nll)$p
  else optim(c(.7, .7), nll)$p
}

para<-fit_main(dat, intensity, nyes, ntrials, .25, lapses=T, cum_normal_fun)
para
