...
As lapses do occur, the fit doesn't seem good and the threshold biased. The problem is not that the likelihood is not correctly maximized; this *weird* fit is the one with the maximum likelihood, given the lapses (Wichmann and Hill, 2001). To be more secure, that the optimization process is not reaching a local maximum, we repeate the fit using differential evolution algoriths (function `DEoptim`)
...
plotcurves(fitWithLapses) + ylim (0,1.5) # we include ggplot ylim to be able to visualize what is going on
```

The problem is that, as we did not constraint the values of the upper asymptote, the lapse rate is sometimes negative and consequently the fitted curves go through probabilities larger than one, which causes that the parametric bootstrap samples cannot be calculated. For this reason, *quickpsy* indicates that the bootstrap should be non-parametric. We avoid the error by performing a non-parametric bootstrap
...
plotcurves(fitWithLapses) + ylim (0,1.5) 
```
...
fitWithLapses <- quickpsy(qpdat, phase, resp, 
                          grouping = c(ecc, interval ,participant), lapses  = T, 
                          pini = list(c(20, 1000), c(20, 1000), c(0, .1))) 

plotcurves(fitWithLapses) 
```

## Wichmann and Hill simulations

Here we replicate the result of Wichmann and Hill (2001) that when lapses occur the estimated threshold is biased,using simulations like theirs (see the original paper for details).
...
simdat <- merge(s, expand.grid(n = 160, sample = 1:30, lambda = seq(0,.05, .01))) %>%
...
So, fitting the lapses not always eliminate the bias in threshold estimation. 
